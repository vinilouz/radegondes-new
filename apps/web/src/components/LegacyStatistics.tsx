...
import { subDays, format, startOfDay, millisecondsToMinutes } from 'date-fns';
import { ptBR } from 'date-fns/locale';

// Componente que renderiza as estatísticas "legadas" ou que não usam getStatisticsMetrics
export function LegacyStatistics({ selectedPeriod }: { selectedPeriod: number | 'all' }) {
  ...
  // Formata dados para o gráfico de produtividade na semana
  const chartData = (() => {
    if (!stats?.dailyData) return [];
    const dataMap = new Map(stats.dailyData.map(item => [item.date, {
      date: format(new Date(item.date + 'T12:00:00'), 'dd/MM', { locale: ptBR }),
      durationMinutes: millisecondsToMinutes(item.duration),
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      if (dataMap.has(dateStr)) {
        ...
        allDates.push({ date: format(date, 'dd/MM', { locale: ptBR }), durationMinutes: 0, durationMs: 0 });
      }
    }
    return allDates;
  })();
  ...
  const calculatedMaxScale = Math.max(720, maxDurationMinutes + 60);
  ...
  function formatstudedTime(ms: number): string {
  ...
  return (
    <>
      {/* Cards de Métricas Principais (da query antiga) */}
      {/* <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <Card className="border ">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">maior soma de tempo estudado</p>
          </CardContent>
        </Card>
      </div> */}

      {/* Gráfico de Produtividade na Semana */}
      {/* <div className="my-8">
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData}>
                    ...
                    />
                    <Tooltip
                      formatter={(_value: any, _name: any, props: any) => [formatHoursMinutes(props.payload.durationMs), 'Tempo de Estudo']}
      ...
      {/* Rankings e Disciplinas de Hoje */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border ">
          <CardHeader><CardTitle>Disciplinas estudadas</CardTitle></CardHeader>
                ...
                {stats.disciplineStats.map((discipline, index) => (
                  <div key={discipline.name} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">{index + 1}</div>
                      <div>
                        <p className="text-sm font-medium">{discipline.name}</p>
                        <p className="text-xs text-muted-foreground">{discipline.sessions} sessões</p>
                      </div>
                    </div>
                    <span className="font-mono text-sm font-semibold">{formatstudedTime(discipline.time)}</span>
                  </div>
                ))}
              </div>
            ) : (
        ...
        <Card className="border ">
          <CardHeader><CardTitle>Disciplinas estudadas hoje</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-4">
              {todayDisciplinesQuery.data && todayDisciplinesQuery.data.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-muted-foreground">Nenhuma disciplina estudada hoje</p>
                </div>
              )}
            </div>
          </CardContent>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats?.disciplineStats.map(discipline => ({
                  ...
                  taxaAcertos: discipline.correct && discipline.wrong ? Math.round((discipline.correct / (discipline.correct + discipline.wrong)) * 100) : 0
                })) || []} margin={{ top: 20, right: 10, left: 10, bottom: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="name" tick={{ fontSize: 10 }} angle={-45} textAnchor="end" height={80} />
                  <YAxis tick={{ fontSize: 10 }} label={{ value: 'Acertos (%)', angle: -90, position: 'insideLeft', dy: 30 }} domain={[0, 100]} />
                  <Tooltip
                    formatter={(value) => [`${value}%`, 'Taxa de Acertos']}
                    labelFormatter={(label) => `Disciplina: ${label}`}
                  />
                  <Bar dataKey="taxaAcertos" fill="#22c55e" name="Taxa de Acertos" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
